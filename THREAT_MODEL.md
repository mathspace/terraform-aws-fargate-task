...
| --- | --- | --- | --- | --- | --- | --- |
| service | Task container | image URL/tag + environment/secrets → definition.sh | ${image_repository_url}:${image_tag}; env values; secret references as valueFrom | Runner-local principals permitted to inspect process command lines during evaluation, container, ECS agent, Terraform state/backend readers and principals permitted to describe task definitions (literal environment values); resolved secrets have separate ECS retrieval controls | Distinct execution/task IAM; caller security groups | main.tf:17-23; definition.sh:26; main.tf:33 |
...
| Terraform deployment | Service capacity/network | caller vars → ECS service | cluster ${cluster_name}, service ${name}; desired_count 0 default; CPU 256/memory 512 defaults; caller subnets/security groups | ECS/Fargate and destinations reachable through caller subnets, routes and security-group egress | AWS ECS APIs, IAM and caller VPC/egress controls; no outbound isolation is supplied by lack of an inbound public endpoint | main.tf:45-59; variables.tf:22; examples/python-hello-world/vpc.tf:22-43 |
...
Caller image and configuration become a single essential container. Environment is JSON-encoded directly into definition.sh process arguments, readable during evaluation by same-host principals with command-line inspection permission, and contains literal values in the external data result and registered container definition, exposing them to Terraform state/backend readers and ECS DescribeTaskDefinition principals as well as the runtime (main.tf:22-30; definition.sh:26-38). Keep sensitive values out of plain environment, isolate the runner and restrict process inspection and state/task-definition access. Secrets remain references in the task definition and are resolved by ECS under externally supplied permission. Task networking is caller-owned through subnets/security groups and external routing. Reachable Internet endpoints and routed peers are potential recipients of workload traffic; a compromised container can exfiltrate injected secrets if egress permits. Constrain egress where isolation is required. The module defines no listener, route, load balancer or public endpoint, and creates no Cloudflare resources (definition.sh:31, main.tf:56).
...
The supplied example uses a private subnet, NAT gateway and all-protocol 0.0.0.0/0 security-group egress with a hello-world image, explicitly allowing routed outbound traffic; it does not establish production network exposure. The module supplies thirty-day log retention, but caller-controlled log content and external access policies determine confidentiality. The README recommends the manual helper and advertises secret injection; both require the caller obligations described above. No real secret, invocation policy or application authorization implementation was inspected (examples/python-hello-world/vpc.tf:14-43, main.tf:62, README.md:15, README.md:30).
